 ...
 *        It takes as input paths generated by the Pathfinder class. There are a
 *        number
 *        of adjustable parameters that can be set when creating the paths that
 *        will
 *        control the behavior of the Pure Pursuit. See the Pathfinder class for
 *        details.
 *
 */
public class PurePursuit {
	...
	/**
	 * 
	 * @brief SetSpeed defines the interface used by PurPursuit to set the speed of
	 *        the motors.
	...
	private double k_minLookAheadDist = 0.6; // Minimum allowed look ahead distance (overrides look ahead time if
												// necessary)
	private double k_maxSearchTime = 1; // Max time along the path to search for the closest point to the current
										// position
	...
	private int k_maxLookAhead = (int) (k_maxSearchTime / m_dt); // Max index count to look ahead for closest point to
																	// current position

	// private final int k_checkStall = 40; // Distance from the end of the path to
	// check for stall - MUSTFIX - with min speed is this really necessary?
	...
	private boolean m_isExtended; // If true, for the curvature calculation, the path is extended by
									// k_extendedLookAhead tangent to the ending point
	...
	private BooleanSupplier m_cancel; 	// Cancel command
	private final double k_maxSpeed = 5; // Max Speed of the robot

	/*
	 * Logging fields
	 */
	private Field[] k_fields = { new Field("Yaw", 'f'), new Field("Vel", 'f'), new Field("Ideal Vel L", 'f'),
			...
			new Field("Distance", 'f') };
	// new Field("Update Count", 'd'), new Field("Errors", 'd') };

	private CSVWriter m_writer; // Used for logging

	/**
	 * @param navigator - Specifies the Navigator class to be use to obtain the
	 *                  robot's position.
	 * @param setSpeed  - Specifies the callback function used to set the robot's
	 *                  speed. This function should accept the speed in feet/second.
	 ...
	 * @param isExtended  - If true, the path is extended beyond the end, tangent to
	 *                    the ending angle, for the purpose of tracking (the robot
	 *                    will still stop when the original end is reached.)
	 * @param setPosition - If true, set the starting position and angle based on
	 *                    the first point of the path.
	 */
	public void loadPath(Path path, boolean isReversed, boolean isExtended, boolean setPosition, DoubleSupplier speed, BooleanSupplier cancel) {
		synchronized (m_dataLock) {
			...
			k_maxLookAhead = (int) (k_maxSearchTime / m_dt); // Max index count to look ahead for closest point to
																// current position
			Logger.log("PurePursuit", 1,
					String.format("Look Ahead Time = %f, Look Ahead Index = %d", k_lookAheadTime, k_lookAheadIdx));
		}
	}

	/**
	 * Enables logging of the robots motion along the path to the robot's local file
	 * system.
	 * 
	...
	/*
	 * The path can optionally end with a non-zero velocity. This function retrieves
	 * that velocity.
	 */
	...
	/*
	 * This function computes the left and right motor speeds required to follow the
	 * path
	 */
	private SpeedContainer followPath() {
		...
		// Find the index of the closest point on the path to the robot's current
		// position
		...
		// if (closestPathIdx > m_path.m_centerPath.length - k_checkStall &&
		// !m_isReversed) {// was 50
		...
		if (m_speed != null) {
			...
			if (speed < 0) {
				...
				return new SpeedContainer(k_maxSpeed * speed, k_maxSpeed * speed);
			} else {
		...
		if (distance < k_minLookAheadDist) {
			...
			if (nextPosIdx < 0 && m_isExtended) {
				...
				double newX = newPos.x
						+ extendDistance * Math.cos(m_path.m_centerPath[m_path.m_centerPath.length - 1].heading);
				double newY = newPos.y
						+ extendDistance * Math.sin(m_path.m_centerPath[m_path.m_centerPath.length - 1].heading);
		...
		// System.out.println(String.format("right = %f, left = %f", rightSpeed,
		// leftSpeed));
	...
	// }
	/*
	 * This function finds the closest point on the path to the current robot
	 * position
	 * It starts searching at the last closest point and will search a maximum of
	 * k_lookAheadPoints
	 * 
	 */
	...
	/*
	 * This function finds the next point on the path that is lookAheadFt distance
	 * from the current robot position
	 * It starts looking at the closest path point to the current robot position and
	 * continues until
	 * it finds a point that is the correct distance or until the end of the search
	 * is reached
	 */
